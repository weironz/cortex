...
/// ⚠️ **枚举声明顺序 = 左栏展示顺序**（`_rows` 按这里的顺序组段）。
/// 2026-08-24 按设计稿重排为 置顶 / 项目 / 最近 —— 置顶的定义就是
/// 「用户要它在最上面」，压在项目下面等于置了个寂寞；「最近」固定最下、
/// 默认展开，它是绝大多数进来的人要找的那一段，放底部离新增内容最近。
enum SidebarSection {
  /// 置顶的**会话**，平铺。
  pinned('pinned', '置顶', '把对话置顶，它会出现在这里'),

  /// 置顶的**项目**，展开是它下面的会话。
  projects('projects', '项目', '把项目置顶，它会出现在这里'),

  /// 其余会话，按时间倒序 —— 所以叫「最近」。
  /// 未置顶的项目仍然在这一段里以分组标题出现。
  chats('chats', '最近', '还没有对话');
