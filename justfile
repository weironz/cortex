...
# ── **节点上**那六件人工操作不在这里，在 docs/backup.md ──────
#
# 体检 / 立刻备一次 / 恢复演练 / 从转储捞一张表 / 灾难恢复 / 灌回 blobs ——
# 全部逐步写在 [docs/backup.md]「人来操作：先认清自己在哪一格」。
#
# **刻意不在 justfile 里复述**，两个理由：
#
# 1. 节点上**没有 just**（也没有仓库，只有 ansible 送过去的 `scripts/`）。
#    一条在那台机器上敲不出来的 recipe，是把人往错的方向带。
# 2. 复述一遍就是第二处装配 —— 而这一类东西改了一处漏另一处的症状，
#    是出事那天照着一份过期的步骤操作。
#
# 这里只留开发机上用得着的：`just drill` 打的是**本机**那套库。
