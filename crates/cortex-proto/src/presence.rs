...
pub struct AttachOffer {
    // ⚠️ **这里曾经有一个 `addr`（直拨地址），2026-08-30 随直拨整条删掉。**
    //
    // 老 worker 仍然会在心跳里带上它 —— serde 默认忽略不认识的字段，
    // 所以那些心跳照常被接受，只是那个地址不再有任何作用（它们也接不进来，
    // 见 `cortex-agentd` 的 presence 模块）。**不要为了「兼容」把字段加回来**：
    // 一个被读进来又不参与任何判断的字段，正是下一次「这个配置改了没反应」。
    /// 接入面专用的钥匙。见结构体文档。
    pub token: String,
}
