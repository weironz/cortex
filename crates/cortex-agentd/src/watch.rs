...
///
/// 不再检查「有没有 docker」：这个进程连不上 docker 就根本不启动
/// （见 `main.rs` 的 preflight），走到这儿时 runner 一定是活的。
pub fn spawn_oom_watch(st: AgentState) {
...
/// 连一次事件流，读到断为止。
async fn watch_once(st: &AgentState) {
    let Some(events) = st.runner().watch_oom() else {
...
/// 定时数一遍每个沙箱的卷占用。
pub fn spawn_quota_watch(st: AgentState) {
    tokio::spawn(async move {
...
async fn sweep_quota(st: &AgentState) {
    // 遍历作用域而不是用户：一个用户的几个项目各有一个卷，配额也各算各的
    for key in st.scopes() {
        match st.runner().workspace_bytes(&key).await {
            Ok(Some(bytes)) => {
