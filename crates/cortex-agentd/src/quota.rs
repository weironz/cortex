...
impl AgentState {
    ...
    pub async fn enforce_quota(&self, user_id: &str) -> Result<(), ApiError> {
        ...
        // 说清楚超了多少、什么时候恢复。一句「配额已用完」会让人以为
        // 账号坏了，然后去重试、去重启、去提 issue。
        //
        // ⚠️ **这段话不许带 markdown。** 它落在客户端一条红色横幅里，
        // 那儿是个纯 `Text`，不渲染 —— 用户看到的是字面的星号。
        // 2026-08-26 实地撞到：屏幕上写着「在**设置 → 模型 → 自己的
        // API key**里填一把」。
        //
        // ⚠️ **路径要与界面对得上。** 那句里的「设置 → 模型」早就改叫
        // 「模型服务」，而「自己的 API key」那个单槽概念也已经变成一份
        // 可增删的来源列表。指一条不存在的路比不指更糟：用户会在设置页里
        // 翻上一分钟，然后以为是自己找不到。
        //
        // 顺序也换了：先说这一轮怎么办（换一条来源，按钮就在这条错误
        // 旁边），再说没有来源的人去哪加。原来那句只有后者，而一个已经
        // 配好几条来源的人读到的是「去加一把你早就加过的 key」。
        Err(ApiError::too_many_requests(exceeded_message(&status)))
    }
}

/// 超额时给用户看的那一句。
///
/// # 为什么它是一个函数而不是就地 `format!`
///
/// 它有两条**只有人能验、机器不报错**的要求（不带 markdown、指得出界面上
/// 真有的路），而验它的测试如果自己抄一份格式串，抄的那份与真的那份会各自
/// 演化 —— 于是测试永远绿，屏幕上永远错。这个仓库反复吃过这个形状：
/// 「验证工具自己造出通过」。
///
/// 提成函数之后，测试打的就是发给用户的那一份。
fn exceeded_message(status: &QuotaStatus) -> String {
    // ⚠️ **不许带 markdown。** 它落在客户端一条红色横幅里，那儿是个纯
    // `Text`，不渲染 —— 用户看到的是字面的星号。2026-08-26 实地撞到：
    // 屏幕上写着「在**设置 → 模型 → 自己的 API key**里填一把」。
    //
    // ⚠️ **路径要与界面对得上。** 那句里的「设置 → 模型」早就改叫
    // 「模型服务」，而「自己的 API key」那个单槽概念也已经变成一份可增删
    // 的来源列表。指一条不存在的路比不指更糟：用户会在设置页里翻上一分钟，
    // 然后以为是自己找不到。
    //
    // 顺序也是有意的：先说这一轮怎么办（换一条来源，按钮就在这条错误
    // 旁边），再说没有来源的人去哪加。原来那句只有后者，而一个已经配好
    // 几条来源的人读到的是「去加一把你早就加过的 key」。
    format!(
        "这个月的用量已经用完了（{} / {} token，按最近 {} 天滚动计算）。         等最早那批用量滑出窗口就会自动恢复。         要立刻继续：点右边的「换模型」，挑一条你自己的来源 ——          自带 key 的调用不占这里的配额。         还没有自己的来源的话，去 设置 → 模型服务 加一条。",
        status.used_tokens,
...
mod tests {
    ...
    /// **超额那条错误是给人读的，所以它有两条硬性要求。**
    ///
    /// 1. **不许带 markdown。** 它落在客户端一条红色横幅里，那儿是个纯
    ///    `Text`。带了的表现是屏幕上出现字面的星号 —— 2026-08-26 实地
    ///    撞到，用户截图问「这又怎么了」。
    /// 2. **提到的路径要在界面上找得到。** 那句原来写着「设置 → 模型 →
    ///    自己的 API key」，而那一层早就改叫「模型服务」，单槽 key 也变成了
    ///    一份可增删的来源列表。指一条不存在的路比不指更糟。
    ///
    /// 两条都不会有任何编译期或运行期信号 —— 服务端把字符串发出去，
    /// 客户端照单画出来，双方都「正常」。
    #[test]
    fn 超额那条话不带markdown_且指得出界面上真有的路() {
        let msg = exceeded_message(&status(2_000_879, Some(2_000_000)));

        assert!(
            !msg.contains("**"),
            "这句话里有 markdown 星号 —— 客户端那条横幅不渲染，用户看到的就是星号本身"
        );
        assert!(
            msg.contains("模型服务"),
            "指路要指界面上真有的那一页。「设置 → 模型」这一层已经不存在了"
        );
        assert!(
            msg.contains("换模型"),
            "要先说这一轮怎么办 —— 那个按钮就在这条错误旁边。只说「去加一把 key」的话，             一个已经配好几条来源的人读到的是「去加你早就加过的东西」"
        );
        assert!(
            msg.contains("2000879") && msg.contains("2000000"),
            "数字要说出来"
        );
    }
