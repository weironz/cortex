    ...
    for key in st.idle_scopes(IDLE) {
        match st.runner().stop(&key).await {
            Ok(()) => {
                // ── 这里**不作废令牌**，是想清楚的 ──
                //
                // 原先会作废（那时令牌注册表就在同一个进程里）。拆开之后作废
                // 要打一次 HTTP，而这条后台路径**手上没有用户凭据** ——
                // 它扫的是所有人的容器，而签发那条端点只认「用户自己那把
                // bearer」。给这个进程配一把能替任意用户作废的服务密钥，
                // 是为了一件几乎没有收益的事新开一个越权面。
                //
                // 没有收益的理由：容器一销毁，那把钥匙就只剩 cortexd 的
                // 注册表里一条记录 —— 它的两个持有者（容器的 env、这个进程
                // 手上那份）都没了，而注册表本身有 TTL 会清掉它。
                //
                // 顺带治好了一个旧隐患：原来那段注释写着「顺序不能反，先作废
                // 再停会让容器正在写的 episode 拿 403」。不作废就没有这个顺序
                // 问题了。
                st.forget(&key);
