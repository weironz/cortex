...
//! # 空闲怎么判
//!
//! 见 [`crate::state::AgentState::idle_scopes`] —— 用「这个作用域多久没有真的
//! 进过容器」，而不是「SSE 连接断了多久」。后者会停掉正在产出的沙箱：
...
/// 起一个后台任务，周期性停掉空闲沙箱。
///
/// 不再检查「有没有 docker」：这个进程连不上 docker 就**根本不启动**
/// （见 `main.rs` 的 preflight），所以走到这儿时 runner 一定是活的。
pub fn spawn(st: AgentState) {
...
async fn sweep_once(st: &AgentState) {
    for key in st.idle_scopes(IDLE) {
        match st.runner().stop(&key).await {
            Ok(()) => {
                // ── 这里**不作废令牌**，是想清楚的 ──
                //
                // 令牌表 2026-08-16 又回到了这个进程里
                // （`crate::delegated_token`），所以「作废要打一次 HTTP、而这
                // 条后台路径手上没有用户凭据」这个旧理由已经不成立了。
                // 结论没变，换了一个更硬的理由：
                //
                // **签发是按作用域复用的**（`routes::delegate_token`）。作废
                // 之后，同一个作用域下一次会拿到一把新令牌，而
                // `runner::ensure` 的 `token_matches` 认的是容器启动时 env 里
                // 那把 —— 判否，于是它把容器强制重建。停掉的容器下次本来就要
                // 重建，但**停之外的失败**（就绪超时那类）也走这条判断，而
                // `ensure` 的文档明写着「容器还在……重试会复用它」。
                //
                // 不作废的代价只有一条过期前占位的记录，而那张表的 TTL 会清掉
                // 它。顺带也没有「先作废再停会让容器正在写的 episode 拿 403」
                // 那个顺序问题。
                st.forget(&key);
