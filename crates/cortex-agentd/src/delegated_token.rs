...
impl DelegatedScope {
    ...
    pub fn allows(&self, method: &axum::http::Method, path: &str) -> bool {
        ...
        match (method, path) {
            ...
            // 技能正文（`load_skill` 工具）。**只放取正文那一条** ——
            // `GET /skills`（整张表，含所有正文）不放：那是设置页的路，
            // 容器里没有任何理由把全部正文一次拉走。
            //
            // 漏掉这条的表现与生图那条一模一样：桌面端取得回来，云端会话
            // 取不回来，而两者跑的是同一份 agent 代码。实测过 —— 模型调了两次
            // `load_skill`，两次 403，然后它老老实实地告诉用户「取不回来」
            //（错误文案是对的），但那条技能在云端等于不存在
            (&Method::GET, "/skills/body") => true,
