...
pub fn brief(base: &str, workspace: Option<&str>) -> String {
    match workspace {
        Some(path) => format!(
            "{base}\n\n当前会话的工作区是 `{path}`。相对路径相对于这个目录。\
             工作区**之外**的位置（桌面、下载、别的项目目录）你也可以访问，\
             但必须用完整的绝对路径，而且会弹一次确认让用户当场批准；\
             他批准之后，那个目录下的后续操作本次会话内不再打扰他。\
             所以：用户说「桌面上的 X」时，**用桌面的绝对路径去做**，\
             不要拿工作区里名字相近的文件顶替，也不要说自己做不到。\
             拿不准绝对路径就先问用户要，别猜。"
        ),
        None => format!(
...
mod tests {
    ...
    fn bound_brief_names_the_absolute_path() {
        ...
        assert!(
            ...
            concat!(
                "提示词里必须出现工作区的绝对路径，否则模型分不清「桌面」与「工作区」",
                "是不是同一个地方，实际：{out}",
            ),
            out = out,
        );
        assert!(
            out.starts_with("底座"),
            concat!(
                "边界说明是**接在**各自的 base 后面，不是替换掉它 —— ",
                "两侧的 base 措辞不同且允许不同，实际：{out}",
            ),
            out = out,
        );
        assert!(
    ...
    fn unbound_brief_does_not_overpromise() {
        ...
        assert!(
            ...
            concat!(
                "线上那句是「绑个工作区我就能帮你在桌面上建 test.txt」—— ",
                "绑完仍然不能，因为围栏挡的是范围。实际：{out}",
            ),
            out = out,
        );
    }
...
",
        ...
        assert_eq!(
            ...
            boundary(&web),
            concat!(
                "边界说明必须逐字相同。两侧各写一份的话，「桌面能不能碰」",
                "会随入口给出不同答案 —— 那是最难归因的一类差异",
            )
        );
    }
}
