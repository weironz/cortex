...
/// # 为什么非讲不可
///
/// 没有这一段时，模型写完一份方案的默认动作是**把正文抄进回复里** ——
/// 一份三千字的方案糊在聊天框里，用户拿不到文件，也不会进资料库。
/// 而 `deliver_file` 摆在工具目录里它未必会用：一个没被提到的工具，
/// 模型只有在自己想起来时才调。
///
/// 反过来，这一段与那个工具**同生共死**（CLAUDE.md 约束 2）：讲了却没有
/// 工具，模型会答应「我把文件发给你」然后什么都没发生 —— 而用户完全
/// 看不出为什么。判据都是同一个 `Caps::can_deliver`。
///
/// # 为什么要专门说「过程文件不要交」
///
/// 模型对「这是不是交付物」一贯偏松。不说的话，一次任务写出来的脚本、
/// 中间 csv、日志会被一起交上来 —— 资料库被灌满，而它唯一的用途
/// （`library_search` 找得到东西）跟着一起垮。
pub const fn deliver_note() -> &'static str {
    concat!(
        "## 做完了要交出去\n\n",
        "用户让你产出一份东西（方案、报表、幻灯片、文档、数据文件）时，",
        "写完文件之后要用 `deliver_file` 把它交出去 —— 那样它会作为附件出现在",
        "回复里，用户能直接下载，也会进资料库供以后检索。\n\n",
        "**不要把文件正文抄进回复。** 一份长文档糊在聊天框里，用户拿不到文件；",
        "回复里只说一句这份东西是什么、有哪几部分就够了。\n\n",
        "**只交最终产物。** 你干活过程中写的脚本、中间数据、日志、构建产物不要交，",
        "它们留在工作区里，用户需要时会自己去看。",
    )
}
