...
/// # 为什么有的路径不给卷根也能用
///
/// `GetLongPathNameW` 的名字查找**不是每一级都做**：一个组件的名字如果
/// 不可能是 8.3 短名（太长、带连字符之类），它就不必去父目录里查 ——
/// 那一级于是不需要列举权。实测：
///
/// | 路径 | `D:/` 未授权时 |
/// |---|---|
/// | `D:/cortex-probe/ws` | ✅ 通（`cortex-probe` 12 个字符，不可能是短名） |
/// | `D:/codes/cortex` | ❌ 挡（`codes` 是 8.3 形状，必须去 `D:/` 里查） |
///
/// 这条解释了之前所有看起来矛盾的观察 —— 包括「同样的 ACE，
/// `D:/cortex-probe` 能用而 `C:/Users/willz` 不能」（`willz` 也是 8.3 形状，
/// 要去 `C:/Users` 里查，而那一级授不上）。
///
/// 而**真实项目目录的名字大多是短的**（`codes`、`src`、`work`），所以
/// 实践中卷根那条 ACE 多半还是要的 —— 它需要管理员（实测 `SetFileSecurityW`
/// 对 `C:/`、`D:/` 都回 `ERROR_ACCESS_DENIED`，那不是传播的锅，是 WRITE_DAC）。
///
/// 卷根**不算断链**（这个函数从它下面一级开始），因为长名字那种情形不需要
/// 它。代价是：卷根没授时，下面那一级的列举权可能什么都买不到。那是一层
/// 目录名的暴露，远小于「主目录被授」那种，所以按「宁可多授这一层」处理。
///
/// # 它放宽了什么
///
