...
按这一轮的特征硬性排除（要工具的排掉 `tool_call == false`、有图的排掉不认图
的、装不下的排掉），**剩下的按最便宜**。

它挑的不是「最优」。判断哪个模型对某个具体问题答得更好，需要一个评测集与
一个判定器，两样都没有 —— 编一个「智能路由」出来，用户会以为难题被送去了
强模型，而实际分派依据是我们瞎写的。界面文案因此只能写「在能干这活的模型
里挑最便宜的」，而 `model_pick.rs` 的实现与这句话逐字对应。
...
`byo_key` 保存时逐字校验供应商名。让人手打的话，填对完全靠猜中我们内部的
id —— `claude` 不行要写 `anthropic`、`gemini` 要写 `google`、`kimi` 要写
`moonshot`，而反馈要到点了保存才来。

清单挂在 `GET /settings/llm-key` 的 `providers` 上，**不是 `/llm/models`**：
后者开头就是 `st.llm()?`，部署自己那把 key 配坏时它直接报错，而那恰恰是最
需要填自己 key 的时刻，下拉不能跟着一起死。
...
上游那份写着 `${ZHIPU_BASE_URL}`，所以我们自己写了 `zhipu.json`）。把它们
全倒进下拉，等于把「一屏四十张卡片」那种乱换个地方摆。

校验那侧仍按 `available()` 放行，**分歧只允许朝这个方向**：下拉里有的必须
存得进去，反过来不必。`每家都说得出名字和端点` 与 `说要发的那几家一家都没漏`
两条测试盯的就是这个 —— 后者防的是**静默消失**（`catalog()` 对定义缺失、
解析失败、`${VAR}` 模板一律 `filter_map` 掉，不报错）。
