...
**③ 可达性只有一个来源：反向隧道。** 曾经必须给 `--attach-addr`：那时唯一的
到达方式是 agentd **拨出去**，而 NAT / 端口映射 / Docker Desktop 那个属于 WSL
的网关三种情形下，监听地址都不是别人够得到的那个。

2026-08-27 反向隧道落地之后这条反过来了：worker **主动拨出**一条常驻长连，
agentd 把请求经它送回去 —— 绑 `127.0.0.1` 的桌面端照样能被 Web 接进来。
于是「`--allow-remote-attach` 与 loopback 绑定互斥、启动即拒」那条校验撤了：
它会把隧道最主要的用户挡在门外。

**2026-08-30 直拨整条删掉**（`--attach-addr`、心跳里的地址字段、心跳路径上那次
外拨探活、`AttachRoute.direct`、那档 409、以及为它建的量具表）。判据三条一起
成立：量具表**一行都没写过**（它零计数时主动不写，所以空表就是「没人接入过」）、
生产上远程接入**一次都没被启用过**（两个 agent 都不带 `--allow-remote-attach`，
而桌面端本来就从不传 `--attach-addr`，根本产生不出直拨路）、隧道那条有全链路
测试每次构建都跑。

删它买到的不是「少一条用户在走的路」，而是**少一条从没被人走过的回退路** ——
真到隧道断掉那天，一条从来没被验过的路多半也是坏的。

**④ 那句 409 分成三档。** 「在线但没开放接入」（附带怎么开）、「只靠隧道而隧道
刚断」（**什么都不用改，等它回来**）、「没有任何在线 agent 持有它」。

第二档是隧道带来的那一档，也是最容易被合并掉的那一档：心跳 TTL 是 90 秒而
隧道靠 h2 PING 几秒就发现断开，于是「机器刚休眠」有一个窗口会被误报 ——
从前误报成「地址配错了」，用户因此去改一个没错的参数。实测撞到过。
（直拨删掉之后那一档不复存在，剩下的两档仍然必须分开：一个要去开开关，
一个只需要等。）
...
出站心跳**不下线**：只想「被看见」而不开放接入的 worker 只有它（隧道只在
`--allow-remote-attach` 打开时才建）。两条路写同一张表，拉那条**只覆盖自己
拉得到的字段** —— 整条覆盖的话，隧道轮询会把心跳刚报的那把接入钥匙换掉。

> 直拨还在的年代，这条规则挡的是另一件事：把心跳报过的直拨地址擦掉，
> 于是回退路在隧道还活着的时候悄悄消失。直拨 2026-08-30 删掉之后
> **这个不变量原样成立**，只是主语从地址变成了钥匙 —— 它防的形状是
> 「一个字段的缺席被当成一次撤销」，与直拨无关。
