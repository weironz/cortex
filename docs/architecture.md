...
它**不是 `String`** —— 用字符串的话，一个手改过数据库的 `"exa"` 会以 Tavily
的形状发给 Exa 的端点，而那种失败在 502 里，说不出「我们不认识这家」。

⚠️ **每家声明自己有哪些能力**（搜索 / 抓正文），这是从 Cherry Studio 的
设置页学到的最有价值的一点。博查只做搜索 —— 界面据此不把它摆进「URL 获取
服务商」，而服务端那侧也有一道（切到它之后 `web_fetch` 回 501 并说清原因，
**不悄悄换一家花你另一份额度**）。
...
Tavily 的 `/extract` 三件全做了，而且**我们已经有它的 key 与那条路**
