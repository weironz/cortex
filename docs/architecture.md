...
> ⚠️ **被否掉的是「工具外派」，不是「服务端那条循环」。**
>
> 服务端循环连同 `Live::workspace_turn`、会话绑定工作区、
> `cortex-prod-workspace` 卷 —— 那整套**留着**，因为它正好是另一个场景：
> **派 agent 去云上干活**（克隆仓库到云上工作区、跑两小时、关掉笔记本也照跑）。
>
> 我们一直把它当缺陷看（「工具动的是服务器的文件」），但那只在
> 「我要它改我本机的代码」这一个场景下才是缺陷。
>
> 所以 D 是**加一条本地路径**，不是**删掉服务端那条**：
>
> | 场景 | 走哪条 |
> |---|---|
> | 改我本机的代码 / 文档 | 本地 agent（D，未建） |
> | 派出去干活，关掉笔记本也照跑 | 服务端循环（**已建**） |
> | 没有在线桌面时随便聊两句 | 服务端循环（已建） |
>
> 而派出去的那个 agent 与你**共用同一个记忆权威** —— 它学到的东西直接进
> 你的记忆，不用「汇报」。Devin 那类云 agent 交付的是一个 PR，
> 过程随容器消失。见 [roadmap G](roadmap.md)。
...
拆成两个进程之后，下面这些从「不存在」变成「必须设计」。
它们不改变架构方向，但会决定落地质量：

| | 问题 | 为什么现在不存在 |
|---|---|---|
| **注入块格式成为线上契约** | `GET /memory/context` 返回**渲染好的块**，于是那个格式从内部实现细节变成**跨进程契约**。本地 agent 与 cortexd 独立发版，格式漂移会静默出错（模型收到一段结构不对的记忆，不报错，只是效果变差） | 现在注入编排与渲染在同一个进程里，改了就一起改 |
| **版本兼容** | 「本地能跑但记忆接口不兼容」是一类全新故障。需要协议版本号 + 明确的不兼容行为（拒绝启动并说清楚，而不是降级运行） | 现在只有一个二进制 |
| **进程健壮性与更新** | 本地进程挂了 = agent 整个不可用。要有崩溃恢复、自动拉起、日志、以及**自动更新**（否则版本碎片化会把上一条放大） | 现在 cortexd 由 docker restart 兜着 |
| **会话执行状态 ≠ 记忆** | 执行到第几步、中间变量在本地进程内存里；记忆在远端。桌面端跑到一半切到 CLI，恢复得了历史，恢复不了执行上下文 | `confirm.rs` 已经对「待确认项」做过同类裁决（**刻意不落库**：进程一重启那一轮的循环与挂起的 future 都没了，此时数据库里一条永远等不到人的待办比不写更糟）。更广的「会话状态要不要同步」**还没想过** |

> 顺带一条**今天就成立**的性质：**抽取是异步的**（`live.rs` 里 `tokio::spawn`，
> 注释写着「结果对本轮毫无用处：它是给下一轮准备的」）。
