...
**agent 循环跟着「文件」走，记忆权威跟着「持久与多端」走。两者之间只有 HTTP。**
...
| **Cormex（记忆服务）** | 任何地方 | 存储、抽取、四路召回、双时间轴回放、MCP 门面、委托凭据签发 | **另一个仓库**：[weironz/cormex](https://github.com/weironz/cormex) |
...
`/api/chat` 与 `/api/sandbox` 进 agentd，其余 `/api` 进 Cormex。
...
|---|---|---|---|
| 渲染工具事件、答工具确认 | ✅ | ✅ | ✅ |
| **操作你机器上的文件** | ✅ 走本机 cortex-local | ✅ | ⚠️ 走**云沙箱**里的 `/workspace`，不是你的磁盘 |
