...
会话可以把根**收窄到卷里的一个命名子目录**（`/workspace/<name>`）：

```
PATCH /sessions/{id}  {"container_workspace": "client-a"}   → 根变成 /workspace/client-a
PATCH /sessions/{id}  {"container_workspace": null}         → 回到卷根（默认）
```

名字由用户起、**可以被多个会话共用** —— 所以这不是「一个会话一个目录」：
按会话分的话，「昨天让你生成的那份报告呢」会得到一个空目录，那正是当初选择
按项目分卷的理由。不设名字时行为与以前**一字不差**（根就是卷根），这是这次
改动唯一重要的安全性质。

目录**第一次用到时才建**（`Workspaces::container_subdir`）。建不出来就回落到
卷根并打一条 WARN —— 让整轮失败的话，一个磁盘满会表现成「这个会话不能用了」。

**形状在三个地方各校一遍**：agentd 的 PATCH（给用户看的错误）、schema 的
CHECK（防绕过 HTTP 面直接写库）、`container_subdir`（离拼路径那一行最近）。
看着冗余，但三道各自防的是不同的绕过。而它不是洁癖：根是 `<卷根>/<名字>`
拼出来的，而沙箱围栏只保证「模型逃不出根」，对**根本身选错了**无能为力 ——
`..` 能把根抬到卷外面，绝对路径能把它换成别的地方，两者都不报错。

> 只允许**一段**、字母数字加 `. _ -`、不以 `.` 或 `-` 开头、长度 1–64。

#### ⚠️ 分组必须在**第一轮之前**落到服务端

服务端是在收到 `/chat` 的**那一刻**去库里读这个会话属于哪个项目的。
`ChatRequest` 里**没有**项目字段，也没有 `POST /sessions` —— 会话行是由第一轮
隐式建出来的。所以「新建会话时选的项目」必须由客户端先 `PATCH /sessions/{id}`
落进库，否则第一轮读到的是 `project_id = NULL`，那一轮就跑在**未分组**容器里。

2026-08-17 真机撞到过：新建项目、发第一句让它写个文件 —— 文件进了默认沙箱；
第二轮起才进项目沙箱，于是**第一轮写的东西再也看不见**。生产上留下的证据是
同一个 owner 名下并排两套容器与两个卷。

客户端原来把这一步放在第一轮**跑完之后**，理由写的是「服务端现在才确实有这
一行会话，PATCH 才有东西可打」。**那个理由是错的**：`PATCH /sessions/{id}`
不要求会话已经存在，它只往事件流里追加一条，状态行随之物化 —— 实测对一个
从未出现过的 id PATCH 回 **200**，响应里带着 `project_id`。404 的是 `GET`
（它要有消息才有 transcript），两者被混为一谈了。

现在客户端在 `send()` 里、紧邻 `_ensureLocalWorkspace` 之后 **await** 这一步，
失败就不发这一轮。有一条测试钉的是**顺序**（「第一次 `chat()` 之前有没有
PATCH 过」）而不是「最终补上了」—— 后者原来那版也满足。

> **同一个形状当天又咬了一次。** 做命名工作区时发现：容器里的 agent 每轮打
> `GET /sessions/{id}` 取工作区名，而那条路由对一条**还没发过话**的会话回
> 404（`session_digest` 是从 episodes 聚合的）。于是名字明明在库里，第一轮却
> 落在卷根。而 `PATCH` 那条路一直支持这种会话 —— 两条路对同一个会话看法不
> 一致，那个不一致就是 bug。现在判据是「有没有生命周期事件」而不是「有没有
> 消息」，真正不存在的会话仍然 404。
>
> **规律：凡是「先设好、再发第一句」的状态，都要检查第一轮读不读得到。**
