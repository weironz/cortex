...
### ⚠️ 这台节点：阿里云内网服务网段 `100.100.x.x` 不可达

2026-08-23 实测。不是本机防火墙（`iptables -S OUTPUT` 默认 ACCEPT、无相关
规则），VPC 网关 `172.18.175.253` 通得好好的（0.127ms），公网出网也正常 ——
但 `100.100.x.x` 整段不通：

| | |
|---|---|
| `mirrors.cloud.aliyuncs.com`（100.100.2.148） | 连不上 |
| 内网 DNS 100.100.2.136 / .138:53 | 连不上 |
| **实例元数据 100.100.100.200** | ICMP 100% 丢包 |

**apt 源已改成公网的 `mirrors.aliyun.com`**（原文件备份在
`/etc/apt/sources.list.bak-20260823`），改后 `apt-get update` 零 Err、
71 MB / 14s。`sources.list.d/` 那四个本来就是公网 HTTPS 源。
代价：apt 流量从内网（免费）变成走 ECS 的出网带宽。

⚠️ **推论：任何依赖实例元数据的方案在这台上都不能用。** 尤其下面
「四、CI 需要的 secret」里提到的那条改进路线 —— GitHub OIDC → 阿里云 RAM
角色 —— RAM 的临时凭据正是从 `100.100.100.200` 取的。真要走那条路，
得先在 VPC / 安全组那边把这个网络问题解决，而那不是这台机器上能改的。

这也是 `provision.yml` 里刷索引与装包**分成两步**的原因：源不通时
`apt-get update` 自己是 `rc=0`（降级用旧索引继续），而 ansible 的
`update_cache: true` 把它当硬失败，于是整个装机停在一句冒号后面是空的
`Failed to update apt cache after 5 retries:`，而它要装的东西本来就都在。
...
> ⚠️ **动手之前先读二节末尾那条**：这台节点连不上 `100.100.100.200`
> （实例元数据），而 RAM 角色的临时凭据正是从那里取的。也就是说这条路线
> 目前在这台机器上**走不通**，前置条件是先解决 VPC / 安全组那边的网络问题。
