...
### MCP 配置路径可注入 —— 一条会因为机器状态变红的测试，变成确定的（归档 2026-08-28）

`local_agent_test` 起的是**真的** `cortex-local`，而它会去读**开发机上
那份真实的 `mcp.json`**。于是这条测试的耗时取决于「这台机器上装了哪些
MCP server、它们今天快不快」—— 2026-08-28 它就这么红过一次：一台 server
卡住，agent 20 秒没报出地址，而代码一个字都没错。

**一条会因为机器状态变红的测试，比没有这条测试更糟**：它红的时候没人信，
绿的时候也证明不了什么。

两件都做完了：

1. `cortex_mcp::config_path` 认 `CORTEX_MCP_CONFIG`，**它压倒执行环境的
   判断**。空串不算配置过 —— 那是本仓库记过 7 次的「空串顶掉默认值」，
   而这里猜错的方向是「读了一个不存在的路径，于是所有 MCP server 静默消失」。
   它同时是运维的逃生口：一台 server 让 agent 起不来时，指一个空文件就能
   先把它拉起来看日志。
2. 确定性的故障注入：`app/test/local_agent_test.dart` 里那条
   「一台装死的 MCP server 不挡启动」—— 一个**接了连接就一个字节都不回**的
   监听器扮演那台 server（最坏情况：连得上，然后永远沉默），断言就绪
   必须远早于 MCP 的 60 秒连接上限。

---

## ~~手头挂着的~~ —— 三条全部做完（归档 2026-08-28）

这一节顶着「手头挂着的」这个标题留到了 2026-08-28，而里面三条早就完成了：
模型能力判定 08-26、备份与恢复演练 08-25、沙箱镜像随 v0.1.13。
**一个说「进行中」的标题比一条过期的待办更误导** —— 读的人会以为有人正在做。
