...
| 云端会话 | ❌ **配不了** | 容器的文件系统每次都是新的，只有 `/workspace` 是持久卷 —— `mcp.json` 应当放那儿（跟着项目走），而不是容器的用户目录 |
| 配置入口 | ❌ 只能手工编 JSON | 一个「MCP 服务器」设置页：加/删、连接状态、每台报了几个工具、`trust` 档位 |

**云端那条是真缺口，不是没做界面。** `McpHub` 接在 `cortex-local` 上，
而容器跑的是同一个二进制 —— 所以机制在，只是没有任何地方能把配置递进去。
不补它的话，「云沙箱 + MCP」这个组合在产品上不存在。
