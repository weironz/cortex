...
| **桌面端自动更新** | ⛔ **仍然完全没有**。版本端点、下载校验、Windows 上替换正在运行的 exe（要先退出再由外部进程替换）、安装器集成 —— 四件事。**刻意没有开工**：做一半的结果是一个会下载但装不上的更新器，而它每次开机提示一次，比没有更糟 |
...
| 事项 | 为什么不在本期 |
|---|---|
| **一次性容器执行环境**（`ExecEnvironment::Container`） | 对齐两家云 agent，Web 端产出文件供下载。生产节点 2 核 3.5G 已跑 19 个容器，要先解决资源；再加生命周期、下载端点、配额，工作量比整个权限模型还大 |
...
前两个是「造好了但没人调用」（6 次）与「空串顶掉默认值」（6 次）。这个已经 3 次。
...
| 位置 | 落地的是什么 | 用户看到什么 |
|
**第 7 次「造好了但没人调用」，也是记录最完整的一次**：发版 zip 一直带着
`cortex-local`，而 `release-package.sh:92` 与 `release.yml:122` 两处注释都写清了
理由 ——「CLI 用户同样需要它，没有它工具动的是服务器的目录」。理由写下来了、
文件打进包了、**接线从来没做**。CLI 默认打 8080，不知道 8090 上有东西。
