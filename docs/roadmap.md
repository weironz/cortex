...
> ⚠️ **「不是新架构」这句只对了一半**（2026-08-15 去代码里核过）。
>
> **执行侧确实成立。** 轮次跑在一个独立的 `tokio::spawn` 里
> （`cortex-local/src/turn.rs:212`），与 SSE 响应解耦；客户端断开后
> `tx.send` 失败只 `break` 掉转发那一层（注释写着「下游断开只该停止转发，
> 不该丢掉已记下的工具调用」），轮次继续跑，episode 照常落进记忆；
> 容器的空闲回收看的是「令牌多久没被用过」，跑着的轮次一直在续命。
> **关掉浏览器，活还在干。**
>
> **观测侧一格都没有。** `SessionDto` 没有「正在跑」这个状态，也**没有任何
> re-attach 端点** —— 事件发往一个已经被丢弃的 channel，人回来只能等
> episode 落库之后重新拉会话。所以「任务状态可见」不是 UI 活，它要么给
> 运行中的轮次开一条可重挂的事件流、要么落一份 run 状态。
>
> **那正是 E 要的同一套机制**，所以 G 与 E 应该一起想、一起排，
> 而不是把 G 当成一件顺手的产品活。
