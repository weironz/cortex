...
| | 缺口 | 不做的后果 |
|---|---|---|
| 文件树没有 mtime | 契约里 entries 只有 name/is_dir/size | 用户没法判断「这是 agent 刚写的那个吗」 |
| 上传没有进度 | 大文件时界面只是卡着 | 工作区文件通常不大，暂不做 |
| 缓存 GC 按 created 排序 | docker 不给「最后使用时间」 | 偶尔多跑一次 setup（不会删掉正在用的 —— 那个有容器引用着，删不掉） |
