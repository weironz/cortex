...
1. **请求路径仍然是单租户。** `cortexd::live` 持有**一个** `Store`，
   而 `TenantPools` 从来没有被请求路径调用过 —— 两个用户共用 `public`。
   隔离的地基建好了、测过了，但没有接上去。
   要做的是：从 access token 解出用户 → 查 `schema_name` → 取那个租户的
   `Store` → 整条请求走它。`Backend::Live` 那个单例是主要障碍。
2. **登录界面还是粘 token 的那个。** `AuthController::signInWithPassword`
   与 `restoreSession` 都写好并测过了，但 `login_gate.dart` 里仍然只有
   一个 token 输入框，没有任何界面调它们。

**没做的第三件：自带 key。** 配额超限的消息里写着「可以在设置里填自己的
API key」，而那个入口不存在 —— 这是一句**已经发出去的空头支票**，
下次要么补上入口，要么改掉那句话。
...
| 事项 | 症状 |
|---|---|
| **本地 agent 的目录没按用户分** | outbox 与 `workspaces.json` 是每设备一份。一台机器上换账号会把两个人的离线队列混在一起，而重放时不会有任何提示 |
| **后台任务只认 `public`** | re-embed 回填、转录回扫、purge 轮转都假设单 schema。新用户的记忆**永远不会被回填** —— 表现只是「语义召回好像差一点」 |
| **桌面端自动更新** | 完全没有。版本端点、下载校验、Windows 上替换正在运行的 exe（要先退出再由外部进程替换）、安装器集成 —— 四件事，做一半比不做糟：一个会下载但装不上的更新器，用户每次开机被提示一次 |
