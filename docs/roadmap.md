...
| 配置入口 | ❌ 只能手工编 JSON | 一个「MCP 服务器」设置页：加/删、连接状态、每台报了几个工具、`trust` 档位 |
...
**更要紧的一条：我们不是 MCP 客户端。** `rmcp` 在依赖里，但只用来借
`Tool` 这个类型，没有任何 client transport。于是第三方的 MCP server
一个都接不进来。

这是**不对称的**：我们信 MCP 这套原语信到把核心资产（记忆）按它组织成
server 交出去，自己却只当供给方，不当消费方。
...
**1（最确定）· 接 MCP 客户端。** 这条不需要任何架构辩论：我们已经在
`/mcp` 那侧证明了这套原语好用。接上 client 之后，「生成图片 / 生成 Word」
那类能力**有可能根本不用我们写** —— 挂一个别人的 MCP server 就有了。
**所以这条直接改变 F 的做法**：先看接进来能白拿多少，再决定自己写哪几个。

**2（中等）· `builtin_specs` + `execute` 换成注册表。** 纯机械重构，
收益是「加工具不改 match」，且是第 1 条的前置 —— 从 MCP 来的工具得能
和内置工具进同一张表、走同一个闸门。
