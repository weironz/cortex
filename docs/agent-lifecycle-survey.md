...
**6. 记住用过的部署，一点切回。** 地址收进小字之后立刻暴露出一件事：
连不上时它会自动展开，而展开之后**没有任何回头路** —— 用户原话是
「进到这里就没办法切换回去了」。加一个「收起」只解决一半，另一半是
**换回另一个部署要凭记忆重打一串 URL**。所以 `AppConfig` 加了
`knownBaseUrls`（最近在前、去重、封顶 6），登录页把用过的其它地址列成
可点的 chip（只显示 host + 非默认端口 —— 完整 URL 在小按钮上会被截断，
而截掉的恰好是区分两个部署的那一半）。**不做 URL 归一化**：
`https://a/api` 与 `https://a/api/` 在服务端可能真是两条路。

**7. 整屏重做。** 用仓库里那份产品图标本尊（`web/icons/Icon-192.png`，
直接引，不再复制一份到 `assets/` —— 那会多一条「改了图标忘了改另一处」
的路），居中排版、宽度收到 380、字段带前缀图标、登录按钮忙碌时转圈而不是
换文字、离线卡片的按钮挪到标题行（那段说明是读完才决定点不点的东西，
一个已经决定离线的人不该为了找按钮先扫四行字）。
...
## 附二：2026-08-21 的重启风暴，以及为此加的四道防护

用户报「dev 桌面端总是连不上 agent、看不到会话」，并且提了一个更根本的
要求：**程序应该能做到自身防护，不允许有破坏性操作**。

### 现场

`agent-launch.jsonl` 里 **13.7 分钟 730 对 spawn/ready**，周期稳定
1.13 秒，**一条 exit 都没有** —— 也就是说 agent 一次都没崩，是我们在杀它。
查的时候第一反应是「冷启动依赖抖动」，被对抗验证推翻了：那个 provider 只
watch 4 个依赖，抖动最多 4 次、几百毫秒内结束，撑不起 13.7 分钟。

真正的环路：

```
远端凭据失效 → 请求 401
  → 客户端把**任何** 401 都当成「本机 agent 凭据错位」
  → invalidate(localAgentOriginProvider) → agent 被停掉重拉
  → ConfirmController 监听到 api 重建 → recover() 拉一次待确认
  → 又 401 → 再杀 …（约 1.13 秒一圈）
```
...
1. **分得清是谁拒的。** 本机 agent 自己拒的 401 带
   `x-cortex-denied-by: local-agent`（`routes.rs` 的 `require_auth`）。
   带 = 重启能治；不带 = 远端不认，重启永远治不了。
2. **自愈动作要有预算。** 401 重启走 `allowRestartKick`：30 秒窗口最多
   2 次。判据将来若再出错，坏也只坏成「重启两次没用」——
   **一个没有预算的自愈动作不是自愈，是把一次故障变成永动机**。
3. **主动停要留痕。** `stop()` 此前什么都不记（它先撤掉退出监听再 kill），
   于是日志里只有 spawn/ready，看起来像 agent 自己活不长，
   而真相是我们在杀它。现在写 `stopped` 事件。
4. **doctor 认得出这个形状。** 两条判据：`stopped` 密度（新日志），
   以及**拉起频率**（一分钟内 ≥5 次，不依赖任何终止事件 —— 老日志与
   下一个还没想到的重启源都算）。拿真实事故日志实测：
   「4 秒内被拉起 5 次 —— 有东西在反复重启它」。

### 顺带修掉的三样

- **冷启动少杀几个 agent。** `localAgentOriginProvider` 此前 watch 整个
  `AppConfig`（`==` 含 `knownBaseUrls` —— 一份与 agent 无关、却在冷启动时
  从磁盘晚到一次的清单）和无条件 watch `localLlmProvider`
  （每次冷启动必然 loading→data 一次，而它只在离线模式下才被用到）。
  收窄成 `select(useMock/baseUrl/offline)` 与「仅离线时 watch」。
- **握手文件不再跨实例误杀。** `_sweepStaleHandshakes` 原来删掉**所有**
  `agent-*.addr`，包括另一份桌面端正在等的那个 —— 对方等到 20 秒超时，
  报「没有报出监听地址」，而它的 agent 其实好好地起来了。现在只删
  自己 pid 前缀的。
- **退避的注释与实现对不上**（写「1s, 2s, 4s…」，实际给不出 1s ——
  调用方先 `consecutive++` 再读 `delay`）。排查时拿错误的节律去比对日志
  会把人带偏，而「周期 1.13 秒」正是这次定位根因的关键证据。

### 根上那道门：单实例

上面全是「一份程序内部」的防护。而这次事故的**土壤**是两份桌面端
（装机版 + dev 构建）同时跑、共用 `%LOCALAPPDATA%\cortex`：两份进程各拿
同一把 refresh token 去轮换，服务端一次性作废旧值 —— **后轮换的把先轮换的
作废**，输家此后每个请求 401。`settings.json` 的读-改-写互相覆盖（「地址
串台」），`workspaces.json` / `outbox.mark` 只有进程内 Mutex。

而窗口标题和图标完全一样，**用户根本看不出自己开了两个**。所以这是程序
该守的门，不是「用户不该那么用」：`main()` 第一行抢
`%LOCALAPPDATA%\cortexpp.lock` 的排它锁（Windows 的 `LockFileEx`，
进程一死内核自动释放，没有 PID 文件那种陈旧问题），抢不到就把已有窗口
带到前台然后自己退出 —— 此时凭据库、settings、agent 一样都还没碰过。

#### ⚠️ 2026-08-21 补：最后那句「自己退出」当时并不成立

初版是 `if (!ensureSingleInstance()) return;`。**从 `main()` 里 return
结束不了这个进程** —— 窗口的消息循环在 C++ runner 里（`windows/runner/`），
原生窗口在 Dart 代码跑起来**之前**就建好并显示了。于是第二份的实际表现是：

* 不 `runApp`，所以确实没碰凭据、settings 与 agent（**要防的数据损坏防住了**）；
* 但进程活着，屏幕上多一扇**永远白屏的 Cortex 窗口**，任务栏两个一样的图标；
* `just app-status` 如实报「跑了不止一个」—— 一个由防护措施自己造出来的告警。

同一处还有第二个错：`_focusExisting` 的注释写着「此刻我们自己的窗口还不
存在」，据此用 `FindWindowW` 取第一个匹配。既然原生窗口已经存在，那个调用
很可能返回**我们自己那扇白窗**，于是「把已有窗口带到前台」变成了
「把白屏置到前台再退出」。改成 `FindWindowExW` 逐个枚举、用
`GetWindowThreadProcessId` 跳过自己的 PID —— 判据只能是进程号，
因为两个构建的标题与窗口类名完全一样（那正是这一切的根源）。

初版之所以被判成「验过了」，是因为当时用 `just app` 起的第二份，
而**那条 recipe 开头就会先关掉在跑的桌面端** —— 于是「进程数保持 1」
是它杀出来的，不是防护做到的。验一个「第二份会自己退出」的机制，
只能直接跑那个 exe。

⚠️ 已知空洞：老版本（≤0.1.15）不抢这把锁，所以「老装机版已开 + 新版再开」
这一种拦不住，要等装机版升上来才闭合。那段时间里
`just app-status` 的「哪个 exe」一栏是唯一的可见性 —— 2026-08-21 就靠它
当场认出误启的装机版，而代价已经付了：两份进程轮换同一把 refresh token，
dev 那份被作废，界面回到登录页。
