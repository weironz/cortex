...
`embedder=fastembed:gpahal/bge-m3-onnx-int8`，73 条铺垫事实 / 116 道题：

| 题型 | R@1 | R@5 | R@10 | R@∞ | MRR | 命中均名次 | 误召@5 | 平均注入 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 全部 | 0.640 | 0.877 | 0.931 | 0.935 | 0.771 | 2.0 | 0.286 | 13.0 |
| 中文语义 | 0.727 | 0.909 | 0.970 | 0.970 | 0.809 | 1.8 | — | 14.0 |
...
| 跨域检索 | 0.400 | 0.667 | 0.800 | 0.800 | 0.518 | 2.9 | — | 13.1 |
| 应召不到 | — | — | — | — | — | — | **0.500** | 10.4 |

逐路（分母 89）：`vector` 覆盖 89 / 独占 2 / R@5 0.911 · `bm25` 81/1/0.733 ·
`episode` 76/0/0.694（默认不入融合）· `graph` 42/0/0.444 ·
`recency` 35/0/0.111（默认不入融合）。

> **这张表与上一版（111 题）不可直接比。** 两处变动：
> ① 加了五道需要跨域桥的题（q112–q116），它们在**默认配置下本来就该挂** ——
> `跨域检索` 从 0.900 掉到 0.667 全部来自这五道，不是检索变差了；
> ② 修了 `fusion::rrf` 的同分定序（见下）。
